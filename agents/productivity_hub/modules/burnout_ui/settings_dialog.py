...
class SettingsDialog(QDialog):
    ...
    def _setup_ui(self) -> None:
        ...
        # Audio & Hotkeys tab
        audio_hotkeys_tab = self._create_audio_hotkeys_tab()
        tabs.addTab(audio_hotkeys_tab, "Audio & Hotkeys")
    ...
    def _create_audio_hotkeys_tab(self) -> QWidget:
        """Create audio and hotkeys settings tab."""
        widget = QWidget()
        layout = QVBoxLayout()
        layout.setSpacing(15)
        ...
        try:
            for i, dev in enumerate(sd.query_devices()):
                if dev['max_input_channels'] > 0:
                    self.mic_device.addItem(f"{dev['name']}", i)
        except Exception:
            pass

        # Set current selection
        current_device = self.settings.get("mic_device")
        if current_device is not None:
            for i in range(self.mic_device.count()):
                if self.mic_device.itemData(i) == current_device:
                    self.mic_device.setCurrentIndex(i)
                    break

        mic_layout.addRow("Input Device:", self.mic_device)
        mic_group.setLayout(mic_layout)
        layout.addWidget(mic_group)

        # Hotkeys settings group
        ...
        # STT Hotkey
        self.stt_hotkey = QComboBox()
        for label, value in hotkey_options:
            self.stt_hotkey.addItem(label, value)

        # Get current STT hotkey
        current_stt = self.settings.get("stt_hotkey", {"caps_lock"})
        if isinstance(current_stt, set) and len(current_stt) == 1:
            current_stt_key = list(current_stt)[0]
            for i in range(self.stt_hotkey.count()):
                if self.stt_hotkey.itemData(i) == current_stt_key:
                    self.stt_hotkey.setCurrentIndex(i)
                    break

        hotkey_layout.addRow("Speech-to-Text:", self.stt_hotkey)

        # STT mode
        self.stt_mode = QComboBox()
        self.stt_mode.addItem("Toggle (press to start/stop)", "toggle")
        self.stt_mode.addItem("Hold (hold to record)", "hold")
        current_stt_mode = self.settings.get("stt_activation_mode", "toggle")
        self.stt_mode.setCurrentIndex(0 if current_stt_mode == "toggle" else 1)
        hotkey_layout.addRow("STT Mode:", self.stt_mode)

        # VC Hotkey
        self.vc_hotkey = QComboBox()
        for label, value in hotkey_options:
            self.vc_hotkey.addItem(label, value)

        # Get current VC hotkey
        current_vc = self.settings.get("vc_hotkey", {"ctrl_r"})
        if isinstance(current_vc, set) and len(current_vc) == 1:
            current_vc_key = list(current_vc)[0]
            for i in range(self.vc_hotkey.count()):
                if self.vc_hotkey.itemData(i) == current_vc_key:
                    self.vc_hotkey.setCurrentIndex(i)
                    break

        hotkey_layout.addRow("Voice Commander:", self.vc_hotkey)

        # VC mode
        self.vc_mode = QComboBox()
        self.vc_mode.addItem("Hold (hold to record)", "hold")
        self.vc_mode.addItem("Toggle (press to start/stop)", "toggle")
        current_vc_mode = self.settings.get("vc_activation_mode", "hold")
        self.vc_mode.setCurrentIndex(0 if current_vc_mode == "hold" else 1)
        hotkey_layout.addRow("VC Mode:", self.vc_mode)

        hotkey_group.setLayout(hotkey_layout)
        layout.addWidget(hotkey_group)

        # Note about restart
        note_label = QLabel("Note: Hotkey changes require app restart to take effect.")
    ...
    def _save_settings(self) -> None:
        ...
        # Audio & Hotkey settings
        self.settings["mic_device"] = self.mic_device.currentData()
        self.settings["stt_hotkey"] = {self.stt_hotkey.currentData()}
        self.settings["stt_activation_mode"] = self.stt_mode.currentData()
        self.settings["vc_hotkey"] = {self.vc_hotkey.currentData()}
        self.settings["vc_activation_mode"] = self.vc_mode.currentData()
    ...
    def _save_to_file(self) -> None:
        ...
        for k, v in self.settings.items():
            if isinstance(v, set):
                serializable_settings[k] = list(v)
        ...
        if settings_path.exists():
            try:
                with open(settings_path) as f:
                    saved = json.load(f)
                    # Convert hotkey lists back to sets
                    if "stt_hotkey" in saved and isinstance(saved["stt_hotkey"], list):
                        saved["stt_hotkey"] = set(saved["stt_hotkey"])
                    if "vc_hotkey" in saved and isinstance(saved["vc_hotkey"], list):
                        saved["vc_hotkey"] = set(saved["vc_hotkey"])
